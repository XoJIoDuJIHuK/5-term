...
namespace ht
{
	...
	(
		const wchar_t* fileName,         // ��� �����
		bool isMapFile)					// true ���� ������� fileMapping; false ���� ������� ����; �� ��������� false
	{
		HtHandle* htHandle;
		if (isMapFile)
		{
		...
		const Element* element)              // �������
	{
		...
		if (freeIndex < 0) {
			if (strlen(htHandle->lastErrorMessage) == 0)
			{
				writeLastError(htHandle, "-- free index out of bounds");
			}
			return false;
		}
		...
		const Element* element)				// �������
	{
		...
		do
		{
			...
			{
				writeLastError((HtHandle*)htHandle, "no more room for elements");
				return -1;
			}
			if (foundElement->key != NULL && memcmp(foundElement->key, element->key, element->keyLength) == NULL)
			{
				writeLastError((HtHandle*)htHandle, "cannot insert element with existing key");
				return -1;
			}
	...
	void wipeLastError(const HtHandle* htHandle)
	{
		memset((void*)htHandle->lastErrorMessage, 0, sizeof(htHandle->lastErrorMessage));
	}
}
